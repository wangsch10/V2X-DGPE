...
from opencood.loss.point_pillar_mmd_loss import MMDLoss
...
class PointPillarUniDistillLoss(PointPillarLoss):
    def __init__(self, args):
        super(PointPillarUniDistillLoss, self).__init__(args)
        self.kd = args['kd']
    ...
    def forward(self, output_dict, target_dict):
        ...
        kd_loss = kd_loss_feature * self.kd['weight'] #self.kd['weight']=0
        mmd_loss_feature=kd_loss_feature * self.kd['weight'] #self.kd['weight']=0

        if self.early_distill != 0:
            print("self.early_distill != 0:******************")
        ...
        if self.kd.get('intermediate_kd', False):
            single_features, record_len, t_matrix = output_dict['feature_before_fusion'], output_dict['record_len'], output_dict['t_matrix']
            split_x = regroup(single_features, record_len)
            B, L = t_matrix.shape[:2] #5,5
            ego_feature, inf_feature, inf_idx = [], [], []
            # print("split_x.shape",len(split_x[0]))

            for b in range(B):
                # print(b,split_x[b].shape[0])
                cav_num = split_x[b].shape[0]
                ego_feature.append(split_x[b][0])
                if cav_num==1:
                    inf_feature.append(split_x[b][0])
                # if cav_num > 1:
                else:
                    inf_feature.append(split_x[b][1])
                    #inf_idx.append(b)
            ego_feature = torch.stack(ego_feature) #[B, C, H, W]
            inf_feature = torch.stack(inf_feature) ##[B2, C, H, W]
            # print('ego_feature',ego_feature)
            # print('inf_feature',inf_feature)
            # if len(inf_feature) > 0:
            #     inf_feature = torch.stack(inf_feature) ##[B2, C, H, W]
            #     # generate overlap mask
            #     overlap_mask = torch.ones(B, 1, H, W).to(ego_feature)
            ...
            #原来用这个计算egofeature的loss #kd_loss_feature = self.FeatureDistillLoss(ego_feature, teacher_feature, gt_boxes_bev_coords, bev_gt_boxes_mask)
            # print('ego_feature',ego_feature.shape)#([4, 2, 100, 252, 256])
            # print('teacher_feature',teacher_feature.shape)
            #print(ego_feature.shape,inf_feature.shape,gt_boxes_bev_coords.shape)

            #计算自车和基础设施之间的分布损失，最小化源域和目标域之间的分布差距
            #mmd_loss_feature = self.MMDDistillLoss(ego_feature, inf_feature, gt_boxes_bev_coords, bev_gt_boxes_mask)
            #################################################################特征补偿图模块的loss################################start
            ...
            ego_feature = ego_feature.permute(0,2,3,1).reshape(N*H*W, C)
            inf_feature = inf_feature.permute(0,2,3,1).reshape(N*H*W, C)
            #print((F.log_softmax(ego_feature, dim=1)).shape)
            mmd_loss_feature = kl_loss_mean(F.log_softmax(ego_feature, dim=1), F.softmax(inf_feature, dim=1))*100
            #mmd_loss_feature = self.jsd_loss(ego_feature, inf_feature)*1000
            #用mmd计算
            #mmd_loss_feature = self.MMDDistillLoss1(ego_feature, inf_feature)
            
            kd_loss += mmd_loss_feature * self.kd['intermediate_weight']
            ...
            #     kd_loss_feature = self.FeatureDistillLoss(inf_feature, teacher_feature_inf, gt_boxes_bev_coords_inf, bev_gt_boxes_mask_inf)
        ...
        if self.kd.get('relation_kd', False):#这个不计算
        ...
        self.loss_dict.update({'total_loss': total_loss.item(),
                              ...
                              'mmd_loss':(mmd_loss_feature * self.kd['intermediate_weight']).item()
                            })
        # self.loss_dict.update({'total_loss': total_loss.item(),
        #                 'kd_loss': kd_loss.item()                    })
    ...
    def jsd_loss(self,p, q, eps=1e-10):
        ...
        m = (0.5 * (p + q))
        # 计算每个分布与均值分布的 KL 散度
        kl_pm = F.kl_div(q.log(), m, reduction='batchmean')
        kl_qm = F.kl_div(p.log(), m, reduction='batchmean')
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, suffix=''):
        ...
        mmd_loss= self.loss_dict.get('mmd_loss', 0)


        print("[epoch %d][%d/%d]%s || Loss: %.4f || Conf Loss: %.4f"
              " || Loc Loss: %.4f || Dir Loss: %.4f || IoU Loss: %.4f || MMD Loss: %.4f  || KD Loss: %.4f" % (
                  epoch, batch_id + 1, batch_len, suffix,
                  total_loss, cls_loss, reg_loss, dir_loss, iou_loss, mmd_loss, kd_loss))
        ...
        if not writer is None:
            ...
            writer.add_scalar('Iou_loss'+suffix, iou_loss,
                            epoch*batch_len + batch_id)
            writer.add_scalar('MMD_loss'+suffix, mmd_loss,
                            epoch*batch_len + batch_id)
    ...
    def MMDDistillLoss(
        ...
        def compute_mmd(x, y):
            n = x.size(0)
            m = y.size(0)
            xx = torch.mm(x.reshape (-1, x.size(2)), x.reshape (-1, x.size(2)).t())
            yy = torch.mm(y.reshape (-1, y.size(2)), y.reshape (-1, y.size(2)).t())
            xy = torch.mm(x.reshape (-1, x.size(2)), y.reshape (-1, y.size(2)).t())
            ...
            return mmd
        # t1=time.time()
        loss_feature_distill = compute_mmd(
            feature_lidar_sample[gt_boxes_indices], feature_fuse_sample[gt_boxes_indices]
        )
        # t2=time.time()
        # print(t2-t1)
        weight = gt_boxes_indices.float().sum()
        loss_feature_distill = loss_feature_distill / (weight + 1e-4)
        return 10*loss_feature_distill
    # 使用方法
    def MMDDistillLoss1(self,feature_lidar_1, feature_lidar_2):
