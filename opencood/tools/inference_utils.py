...
def inference_late_fusion_2model(batch_data, model, model_inf, dataset):
    """
    Model inference for late fusion.
    ...
    for cav_id, cav_content in batch_data.items():
        if cav_id == 'ego':
            ...
            output_dict[cav_id] = model_inf(cav_content)
...
def inference_no_fusion_inf(batch_data, model, dataset, single_gt=True):
    """
    Model inference for no fusion.
    ...
    """
    output_dict_inf = OrderedDict()
    if single_gt:
        batch_data = {1: batch_data[1]}
    for cav_id, cav_content in batch_data.items():
        # if cav_id == 'ego':
        #     output_dict_inf[cav_id] = {}
        if cav_id != 'ego':
            output_dict_inf[cav_id] = model(cav_content)
    # output_dict only contains ego
    # but batch_data havs all cavs, because we need the gt box inside.

    pred_box_tensor, pred_score, gt_box_tensor = \
        dataset.post_process_no_fusion_inf(batch_data, output_dict_inf)
...
def inference_early_fusion(batch_data, history_spatial_features_2d,model, dataset):
    """
    ...
    output_dict['ego'] = model(cav_content,history_spatial_features_2d)
    # print(';;;;;;;;;;;;;;;;;;;;',output_dict['ego']['cls_preds'])
    pred_box_tensor, pred_score, gt_box_tensor = \
        dataset.post_process(batch_data,
                             output_dict)
    ...
    return_dict = {"pred_box_tensor" : pred_box_tensor, \
                    "pred_score" : pred_score, \
                    "gt_box_tensor" : gt_box_tensor
                        # "feature":output_dict['ego']['feature'],\
                            # "draw_flag":output_dict['ego']['draw_flag']
                    }
    if "depth_items" in output_dict['ego']:
        return_dict.update({"depth_items" : output_dict['ego']['depth_items']})
    if "single_features" in output_dict['ego']:
        return_dict.update({'single_features': output_dict['ego']['single_features']})
        #return_dict.update({'feature': output_dict['ego']['feature']})
        # return_dict.update({'domain_att': output_dict['ego']['domain_att']})
        # return_dict.update({'spatial_att': output_dict['ego']['spatial_att']})
    if 'comm_rate' in output_dict['ego']:
        return_dict.update({'comm_rate': output_dict['ego']['comm_rate']})
    return return_dict


def inference_intermediate_fusion(batch_data, history_spatial_features_2d,model, dataset):
    """
    Model inference for early fusion.
    ...
    gt_box_tensor : torch.Tensor
        The tensor of gt bounding box.
    """
    return_dict = inference_early_fusion(batch_data, history_spatial_features_2d,model, dataset)
    return return_dict
