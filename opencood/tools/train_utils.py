...
def create_model(hypes):
    ...
    return instance
def create_model_historybev(hypes):   ###特征补偿图的model，未加入时序模块，用其中的pointpillar模块来提取历史bev特征
    """
    Import the module "models/[model_name].py
    ...
    model : opencood,object
        Model object.
    """
    backbone_name = hypes['model']['core_method']+'_lcfm'
    backbone_config = hypes['model']['args']
    print('backbone_name_lcfm:',backbone_name)
    model_filename = "opencood.models." + backbone_name
    model_lib = importlib.import_module(model_filename)
    model = None
    target_model_name = backbone_name.replace('_', '')
