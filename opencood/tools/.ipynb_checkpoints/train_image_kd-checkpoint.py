...
def train_parser():
    ...
    parser.add_argument("--hypes_yaml", "-y", type=str, default='/home/lixiang/CoAlign/opencood/hypes_yaml/v2xset/camera_kd/lss_intermediate_kd.yaml',
...
def main():
    ...
    ############ For lidar model teacher ##############
    if "kd_flag" in hypes.keys():
        kd_flag = True
        teacher_model_name = hypes['kd_flag']['core_method'] # point_pillar_disconet_teacher
        teacher_model_config = hypes['kd_flag']['args']
        teacher_checkpoint_path = hypes['kd_flag']['teacher_path']
        ...
        print("loading teacher network ckpt from {}".format(teacher_checkpoint_path))
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        if (epoch+1) % hypes['train_params']['save_freq'] == 0 and (epoch+1) >= epoches * 0.75:
