    ...
    class EarlyFusionDataset(cls):
        ...
        def __getitem__(self, idx):
            ...
            for cav_id, selected_cav_base in base_data_dict.items():
                ...
                # sensormissing_test: drop the inf data
                # drop_prob = 0.1
                # if idx % (1/drop_prob) == 0 and cav_id == 1:
                #     continue
            ...
            if self.visualize:
                projected_lidar_list = copy.deepcopy(projected_lidar_stack)
        ...
        def collate_batch_test(self, batch):
            ...
            for cav_id, cav_content in batch.items():
                ...
                if self.visualize:
                    ...
                    # for divide inf and veh lidar pcd
                    # origin_lidar_list = np.array(cav_content['origin_lidar_list'])
                    # ego_lidar = torch.from_numpy(origin_lidar_list[0])
                    # output_dict[cav_id].update({'ego_lidar': ego_lidar})
                    # if origin_lidar_list.shape[0] == 2:
                    #     inf_lidar = torch.from_numpy(origin_lidar_list[1])
                    #     output_dict[cav_id].update({'inf_lidar': inf_lidar})
