...
def getLateFusionDataset(cls):
    ...
    class LateFusionDataset(cls):
        ...
        def get_item_test(self, base_data_dict, idx):
            ...
            # drop_prob = 0.1
            # if idx % (1/drop_prob) == 0 and cav_num >1:
            #     cav_id_list.pop(-1)
            #     cav_num -= 1
        ...
        def post_process_no_fusion_inf(self, data_dict, output_dict_ego):
            data_dict_inf = OrderedDict()
            data_dict_inf[1] = data_dict[1]
            gt_box_tensor = self.post_processor.generate_gt_bbx(data_dict)

            pred_box_tensor, pred_score = self.post_processor.post_process(
                data_dict_inf, output_dict_ego
            )
            return pred_box_tensor, pred_score, gt_box_tensor
