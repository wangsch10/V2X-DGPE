...
class PointPillarBaselineMultiscaleCoalign(nn.Module):
    """
    F-Cooper implementation with point pillar backbone.
    """
    def __init__(self, args):
        super(PointPillarBaselineMultiscaleCoalign, self).__init__()
    ...
    def forward(self, data_dict,history_spatial_features_2d):
        ...
        print(spatial_features.shape)  #[1, 64, 200, 504]    haitory的shape为8,256, 100, 252]
        '''
        history_spatial_features_2d=history_spatial_features_2d.reshape(-1, 64, 200, 504)
        ...
        print(history_spatial_features_2d.shape)

        if record_len==2 and record_len_history==2:
            print(record_len)
            spatial_features[1, :, :, :]=history_spatial_features_2d[1, :, :, :]
            # record_len= torch.tensor([2], device='cuda:0')
        '''
        ########################################################################################################
        if self.compression:
